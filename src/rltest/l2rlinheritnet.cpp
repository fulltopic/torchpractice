...
#include "rltest/l2rlinheritnet.h"
...
void GRUL2RLInheritNet::initParams() {
...
void GRUL2RLInheritNet::loadModel(const std::string modelPath) {
	GRUL2Net::loadModel(modelPath);
}

GRUL2RLInheritNet::GRUL2RLInheritNet(int inSeqLen):
	GRUL2Net(inSeqLen),
	fcValue(2048, 1)
{
...
Tensor GRUL2RLInheritNet::inputPreprocess(Tensor input) {
//	return input.div(4);
	return input;
}
...
vector<Tensor> GRUL2RLInheritNet::forward(vector<Tensor> inputs, bool isTrain) {
	vector<Tensor> l2NetOutput = GRUL2Net::forward(inputs, isTrain);
...
vector<Tensor> GRUL2RLInheritNet::forward (vector<Tensor> inputs) {
	return GRUL2Net::forward(inputs);
}

//vector: inputs, actions, returns
//each vector: {seqLen, others}
// inputTensors[2] = rewards = float[batchSize]
Tensor GRUL2RLInheritNet::getLoss(vector<vector<Tensor>> inputTensors){
	this->train();
...
Tensor GRUL2RLInheritNet::createHState() {
	return std::move(GRUL2Net::createHState());
}

void GRUL2RLInheritNet::reset() {
	GRUL2Net::reset();
	register_module("fcValue", fcValue);
}

GRUL2RLInheritNet::GRUL2RLInheritNet(const GRUL2RLInheritNet& other)
:
			GRUL2Net(other),
			fcValue(2048, 1)
{
...
GRUL2RLInheritNet& GRUL2RLInheritNet::operator=(const GRUL2RLInheritNet& other) {
	return *this;
}


GRUL2RLInheritNet::GRUL2RLInheritNet(const GRUL2RLInheritNet&& other) :
			GRUL2Net(std::move(other)),
			fcValue(2048, 1)
{
