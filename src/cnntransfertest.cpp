...
static void trainTransferDbOverfit(LmdbSceneReader<DbDefs>& reader, NetType& net,
...
template<typename NetType, typename DbDefs>
static void trainTransferDbOverfit(std::vector<LmdbSceneReader<DbDefs>>& readers,
...
//	optimizer.square_average_buffers;
	...
	trainTransferDbOverfit(reader, net, optimizer, sampleNum, seqLen);
}

template<typename NetType, typename TransferredNetType>
static void test5RowsVecDbs(const std::string modelName, const float lr, const int sampleNum, const int seqLen) {
	std::string modelPath = wsPath + "/workspaces/workspace_cpp/torchpractice/build/models/" + modelName;

	TransferredNetType transferredNet;
	torch::serialize::InputArchive inChive;
	inChive.load_from(modelPath);
	transferredNet.load(inChive);

	NetType net(seqLen, transferredNet);
	...
	trainTransferDbOverfit(readers, validReader, net, optimizer, sampleNum, seqLen);
}
...
static void getLoadedNetStructure(std::string modelName, const int seqLen) {
	std::string modelPath = wsPath + "/workspaces/workspace_cpp/torchpractice/build/models/" + modelName;
...
//	getLoadedNetStructure("CNNNet_1585044542.pt", seqLen);
	test5RowsVecDbs<CNNGRUTransfer, CNNNet>("CNNNet_1585044542.pt", lr, sampleNum, seqLen);
}
