...
//		  vh1(torch::nn::GRUOptions(hiddenSize, hiddenSize).num_layers(1).batch_first(false)),
		  ...
		  ag(torch::nn::GRUOptions(iNumInputs, hiddenSize).num_layers(1)),
		  vg(torch::nn::GRUOptions(iNumInputs, hiddenSize).num_layers(1)),
	...
	vector<Tensor> forward(Tensor inputs) {
		auto valueGruOutput = vg->forward(inputs);
		...
		auto actionGruOutput = ag->forward(inputs);
...
//		const int seqLen = std::min((int)inputs[batchSize - 1].size(0), 64);
...
//template <typename OptimzerType>
//static void updateNet(LunarRnnNet& net, OptimzerType& optimizer, LunarLstmStorageType& storage, vector<float> nextValues) {
//	//obsv: step, batch, others
//	vector<vector<Tensor>> datas = storage.getData();
//	vector<Tensor> obsvs = datas[0];
//	vector<Tensor> actions = datas[1];
